...
void SDL::Renderer::Render(//---------------------------------
 ...
 for (auto const &food : foods) {//--------------
  ...
  for (auto const snake : snakes) {//--------------------
   ...
   for (SDL_Point const &point : snake ->body) { //snake.body
  ...
  // Render snake's head
  block.x = static_cast<int>(snake->head_x) * block.w;//----snake.head
  block.y = static_cast<int>(snake->head_y) * block.h;//-----snake.head
  if (snake->alive) {//---------
